...
"""
Module qui permet de manipuler les mots de la langue française
"""
import random
...
def read_data(filename):
    ...
    Lis un fichier de mots et renvoie une liste de mots sans le dernier \n
    """
...
def cherche2(mots, lstart, lmid, lstop, nmin, nmax):
    ...
    Returns:
        set: retourne le sous ensemble des mots commençant par
        une chaine présente dans lstart, contenant une chaine présente dans lmid et
        finissant par une chaine présente dans lstop, 
    ...
    {'alphabétisassiez'}
    """
    return {mot for mot in mots if nmin <= len(mot) <= nmax
            and any(mot.startswith(pre) for pre in lstart)
            and any(m in mot[1:-1] for m in lmid)
            and any(mot.endswith(suf) for suf in lstop)}


def main():
    """
    Teste les fonctions secondaires en les appelant
    """
    mots = read_data(FILENAME)
